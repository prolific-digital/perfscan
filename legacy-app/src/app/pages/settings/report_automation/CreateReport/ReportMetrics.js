...
import { getPoolsList, getSystemMetrics, getPoolsListSys, getReportMetrics } from "services/apiService";
import { getReport, setMetrics } from "store/slices/report_scheduler/addSchedulerSlice";

const ReportMetrics = ({isEditing}) => {
    ...
    const sysID = Report.system.id || 0;
    
    console.log("System ID", sysID);
    useEffect(() => {
        sysID !== 0 &&
        fetchSystemMetrics(sysID, curUser);
    }, [sysID]);
    ...
    const fetchSystemMetrics = async (sysID, curUser) => {
        try {
          setIsLoading(true);
          setIsError("");
          setMetricId(null);
          const response = await getSystemMetrics(sysID, curUser);
          if (response.status === 200) {
            const data = response.data || {};
            setMetricId(data.metric_id);
            setReportMetricsOption(data.metrics_data || {});
    ...
    useEffect(() => {
        const SaveMetrics = setTimeout(() => {
            dispatch(setMetrics(reportMetricsOption))
        }, 5000)

        return () => clearTimeout(SaveMetrics);
