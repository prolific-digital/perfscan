...
const PeriodResponse5250ChartReport = ({reportId}) => {
    ...
    const period1_sdate = moment(response5250Data?.data[0]?.params?.sdata).unix()
    const period1_edate = moment(response5250Data?.data[0]?.params?.edate).unix()
    if( period1_sdate === period1_edate )
    {
         period1 = moment(response5250Data?.data[0]?.params?.sdata).format("MMM Do YYYY")
    }else{
         period1 = moment(response5250Data?.data[0]?.params?.sdata).format("MMM Do YYYY") + ' - ' + moment(response5250Data?.data[0]?.params?.edate).format("MMM Do YYYY");
    }
    // Show only start date if both start and end date are same
    const period2_sdate = moment(response5250Data?.data[1]?.params?.sdata).unix();
    const period2_edate = moment(response5250Data?.data[1]?.params?.edate).unix();
    if( period2_sdate === period2_edate )
    {
        period2 = moment(response5250Data?.data[1]?.params?.sdata).format("MMM Do YYYY")
    }else{
        period2 = moment(response5250Data?.data[1]?.params?.sdata).format("MMM Do YYYY") + ' - ' + moment(response5250Data?.data[1]?.params?.edate).format("MMM Do YYYY");
    }
...
export default PeriodResponse5250ChartReport;