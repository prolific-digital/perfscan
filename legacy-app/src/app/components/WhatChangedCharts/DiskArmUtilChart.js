...
export default class DiskArmUtilChart extends Component {
    constructor(props) {
        super(props);
      ...
      async componentDidMount() {
        ...
        for (var i = 0; i < newData.data.length; i++) {  
          dps1.push({x: new Date(newData.data[i].event_time), y: +(newData.data[i].disk_arm_utilization)});
          dps2.push({x: new Date(newData.data[i].event_time), y: +(newData.data[i].disk_arm_utilization)});
        }
