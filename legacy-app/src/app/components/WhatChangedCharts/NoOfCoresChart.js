...
export default class NoOfCoresChart extends Component {
    constructor(props) {
        super(props);
      ...
      async componentDidMount() {
        ...
        for (var i = 0; i < newData.data.length; i++) {  
          dps1.push({x: new Date(newData.data[i].event_time), y: +(newData.data[i].totalcores)});
          dps2.push({x: new Date(newData.data[i].event_time), y: +(newData.data[i].totalcores)});
        }
