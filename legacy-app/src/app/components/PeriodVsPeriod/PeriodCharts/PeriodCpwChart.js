...
import { fetchAsyncPeriodCPW, getCPWChartData } from "../../../../store/slices/periodVsPeriodCharts/periodCharts/periodCPUChartsSlice";
...
const PeriodCpwChart = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchAsyncPeriodCPW(pqd));
    }
    }, [dispatch,uuid])
  ...
  if(!_.isEmpty(cpwData.data[0]?.data) && !_.isEmpty(cpwData.data[1]?.data)){
    const min_max_1_dp1 = findMinMax(cpw1[0].dataPoints); 
    const min_max_1_dp2 = findMinMax(cpw1[1].dataPoints);

    const min_max_2_dp1 = findMinMax(cpw2[0].dataPoints); 
    const min_max_2_dp2 = findMinMax(cpw2[1].dataPoints);
