...
import {
  fetchAsyncMachinePoolFaulting,
  getMachinPoolFaultingData,
...
const MachinePoolFaulting = ({ activeChartView }) => {
  ...
  const systemName = MachinePoolFaultingData?.data?.params?.server?.split(" ");
  ...
  useEffect(() => {
    if (
      MachinePoolFaultingData.loading === false &&
      !_.isEmpty(MachinePoolFaultingData.data.data)
    ) {
          ...
          <div className="chart_container">
            {activeChartView.isMetricsChart && (
              <ChartView
                ...
                xAxisDateFormat={
                  MachinePoolFaultingMatrics[0].xValueFormatString
                }
                isVisible={activeChartView.isMetricsChart}
                showTotal={false}
                minimum={0}
                stripLineToggle={activeChartView.isShowGuidelines}
                criticalUtilizationValue={MachinePoolFaultingData}
                metricType={"machine_pool_faulting"}
              />
            )}
            ...
            <div className="chat_main1">
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <img
                  ...
                  src="/noData.webp"
                  alt="No data available for Machine Pool Faulting."
                />
              </div>
            </div>
            <div className="chat_main1">
...
export default React.memo(MachinePoolFaulting);
