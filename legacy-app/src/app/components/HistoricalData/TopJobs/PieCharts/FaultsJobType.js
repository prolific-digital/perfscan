...
import {
  fetchAsyncFaultsTopJobsChart,
  getFaultsTopJobsChartData,
...
import {
  fetchAsyncRealTimeFaultsTopJobsChart,
  getRealTimeFaultsTopJobsChartData,
} from "../../../../../store/slices/topJobs/topJobsRealTimeMonitorSlice";
import { useSearchParams } from "react-router-dom";

const CanvasJSChart = CanvasJSReact.CanvasJSChart;
const FaultsJobType = ({ runReportStateValue, print, alertPage }) => {
  let qd = null;
  ...
  const topJobsChartData = useSelector(
    alertPage ? getRealTimeFaultsTopJobsChartData : getFaultsTopJobsChartData
  );
  ...
  useEffect(() => {
    if (!uuid?.loading && uuid.data.uniqueid) {
      dispatch(fetchAsyncFaultsTopJobsChart(qd));
    }
    if (alertPage) {
      dispatch(
        fetchAsyncRealTimeFaultsTopJobsChart({
