...
import {
  fetchAsyncTopJobsChartSyncIOs,
  getSyncIOsTopJobsChartData,
...
import {
  fetchAsyncRealTimeTopJobsChartSyncIOs,
  getRealTimeSyncIOsTopJobsChartData,
} from "../../../../../store/slices/topJobs/topJobsRealTimeMonitorSlice";

const CanvasJSChart = CanvasJSReact.CanvasJSChart;

const SynciosJobType = ({ runReportStateValue, print, alertPage }) => {
  let qd = null;
  ...
  const topJobsChartData = useSelector(
    alertPage ? getRealTimeSyncIOsTopJobsChartData : getSyncIOsTopJobsChartData
  );
  ...
  useEffect(() => {
    if (!uuid?.loading && uuid.data.uniqueid && !alertPage) {
      dispatch(fetchAsyncTopJobsChartSyncIOs(qd));
    }
    if (alertPage)
      dispatch(
        fetchAsyncRealTimeTopJobsChartSyncIOs({
