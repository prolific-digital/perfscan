...
import { fetchAsyncTotalIOsTopJobsChartReports, getTotalIOsTopJobsChartDataReport} from '../../../../../store/slices/reports/topJobsReport/topJobsReportSlice';
...
const TotaliosJobTypeReport = ({runReportStateValue,reportId}) => {
  let qd = null;
  ...
  const getSysId = useSelector(state=>state.filters.system_filter?.id);
    ...
    const topJobsChartData = useSelector(getTotalIOsTopJobsChartDataReport);
  ...
  const [chartData, setChartData] = useState([]);
    ...
    useEffect(() => {
        dispatch(fetchAsyncTotalIOsTopJobsChartReports(reportId));
      }, [dispatch,getSysId]);
    useEffect(()=>{
    ...
    <div>
      {(topJobsChartData.loading) && (_.isEmpty(topJobsChartData.data)) && 
        ...
        }
        {
          ...
          <div style={{textAlign:'center'}}><img style={{height:'50%',width:'50%'}} src="/noData.webp" alt="No data available for Total-IOs Job Chart"/></div> 
        }
    </div>
  )
}

export default TotaliosJobTypeReport