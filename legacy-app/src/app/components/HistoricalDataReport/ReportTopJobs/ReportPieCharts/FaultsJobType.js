...
import { fetchAsyncFaultsTopJobsChartReports, getFaultsTopJobsChartDataReport} from '../../../../../store/slices/reports/topJobsReport/topJobsReportSlice';
...
const FaultsJobTypeReport = ({ runReportStateValue,reportId }) => {
  let qd = null;
...
const topJobsChartData = useSelector(getFaultsTopJobsChartDataReport);
...
useEffect(() => {
    dispatch(fetchAsyncFaultsTopJobsChartReports(reportId));
  }, [dispatch,getSysId]);
  ...
  <div>
    {(topJobsChartData.loading) && (_.isEmpty(topJobsChartData.data)) && 
      ...
      }
      {
        ...
        <div style={{textAlign:'center'}}><img style={{height:'50%',width:'50%'}} src="/noData.webp" alt="No data available for Faults Job Chart"/></div>
      }
  </div>
  )
}

export default FaultsJobTypeReport