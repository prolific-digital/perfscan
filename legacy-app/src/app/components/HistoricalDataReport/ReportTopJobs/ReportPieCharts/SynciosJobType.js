...
import { fetchAsyncTopJobsChartSyncIOsReports, getSyncIOsTopJobsChartDataReport } from '../../../../../store/slices/reports/topJobsReport/topJobsReportSlice';
...
const SynciosJobTypeReport = ({runReportStateValue,reportId}) => {
  let qd = null;
  ...
  if (runReportStateValue === false) {
    ...
    const topJobsChartData = useSelector(getSyncIOsTopJobsChartDataReport);
  ...
  const [chartData, setChartData] = useState([]);
    ...
    useEffect(() => {
        dispatch(fetchAsyncTopJobsChartSyncIOsReports(reportId));
      }, [dispatch,getSysId]);
    useEffect(()=>{
    ...
    <div>
      {(topJobsChartData.loading) && (_.isEmpty(topJobsChartData.data)) && 
        ...
        }
        {
          ...
          <div style={{textAlign:'center'}}><img style={{height:'50%',width:'50%'}} src="/noData.webp" alt="No data available for Sync-IOs Job Chart"/></div>
        }
    </div>
  )
}

export default SynciosJobTypeReport