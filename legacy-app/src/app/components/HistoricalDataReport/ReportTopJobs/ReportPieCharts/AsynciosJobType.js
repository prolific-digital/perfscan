...
import { fetchAsyncTopJobsChartAsyncIOsReports, getAsyncIOsTopJobsChartDataReport} from '../../../../../store/slices/reports/topJobsReport/topJobsReportSlice';
...
const AsynciosJobTypeReport = ({runReportStateValue,reportId}) => {
  let qd = null;
  ...
  if (runReportStateValue === false) {
    ...
    const topJobsChartData = useSelector(getAsyncIOsTopJobsChartDataReport);
  ...
  const [chartData, setChartData] = useState([]);
    ...
    useEffect(() => {
        dispatch(fetchAsyncTopJobsChartAsyncIOsReports(reportId));
      }, [dispatch,getSysId]);
    useEffect(()=>{
    ...
    <div>
      {(topJobsChartData.loading) && (_.isEmpty(topJobsChartData.data)) && 
        ...
        }
        {
          ...
          <div style={{textAlign:'center'}}><img style={{height:'50%',width:'50%'}} src="/noData.webp" alt="No data available for Async-IOs Job Chart"/></div> 
        }
    </div>
  )
}

export default AsynciosJobTypeReport